...
async def correctLevel(ctx, s: str, challenge_names: tuple):
    ...
    if len(matches) > 1:
        ...
        for match in enumerate(matches):
            options.append(interactions.Button(
                ...
                label=f"{match[1]} (#{challenge_names.index(match[1]) + 1})",
                custom_id=f"levelcorrect{match[0]}"
            ))
            print(f"levelcorrect{match[0]}")
